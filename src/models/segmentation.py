# segmentation.py - FIXED VERSION - All Bugs Resolved - It's worked - 21/07/2025
import numpy as np
import pandas as pd
import psycopg2
import argparse
...
def clean_nan(obj):
    """Nan cleanses the values that disrupt Postgresql's JSON format such as infidel."""
    # ... existing code ... 21072025
    return obj

# Round decrease process - 21072025
def round_numeric_values(obj, decimal_places=2):
    """Round all numeric values in nested dict/list structure and handle NaN/inf"""
    if isinstance(obj, dict):
        ...
        if np.isnan(obj) or np.isinf(obj):  
            return 0.0                      
        if np.isnan(obj) or np.isinf(obj):
            return 0.0  # ← Do NaN/inf values 0.0 
...
class CustomerSegmentation:
    ...
    def _create_segment_profiles(self, df: pd.DataFrame, X: pd.DataFrame, labels: np.ndarray):
        ...
        for s in range(self.n_clusters):
            ...
            profile = round_numeric_values(profile, 2)  # Round all values
            profiles[s] = profile
...
# Fixed: Better NumPy converter - errors solved
def convert_numpy(o):
...
def main():
    ...
    # 2.Fixed: Safer more "avg_session_value calculation" - 21072025
    try:
    ...
    # 4.Fixed: Better customer segment preparation - 21072025
    customer_segments = []
    for idx, row in df.iterrows():
        try:
            segment_id = int(labels[idx])
            clean_profile = clean_nan(model.segment_profiles[segment_id]) # added 21072025
            cluster_row = round_numeric_values(clean_nan(model.segment_profiles[segment_id]), 2) # Add round_numeric_values

            # Fixed: Ensure customer_id is properly handled - 21072025
            ...
            # Fixed: Clean segment data JSON - 21072025
            ...
            # Fixed: Create clean, minimal metadata - 21072025
            clean_metadata = {
    ...
    # 5.Fixed: Filter and validate results - 21072025
