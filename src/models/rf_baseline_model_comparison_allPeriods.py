#!/usr/bin/env python3
"""
Model Comparison from Trained PKL Files - University of Bath Academic Standard - version-7 updated
=============================================================================
update-2: Fixed to work with clean_harmonized_rf models
update-3: Logs added by time-date for each run.
update-4: Critial because the training data was corrupted and bad results were obtained.
- EXACT Training Data: JOIN customer_features + promo_label (same query)
- NO Synthetic Labeling: Does not generate new label in Comparison script
- RF Model's Exact Encoder: Using Model's own LabelEncoder
- Same Preprocessing: Same feature selection, same scaling
- Enhanced Debugging: Detects label mismatch situations
what I have done; (28072025)
- Synthetic Probabilistic Labeling Removed
- Direct promo_label is used from Database
- Exact compatibility with Model's Encoder
- Enhanged Error Logging

update-5: Fix again 
- Applies balanced resampling just like training
- Ensures 4-class distribution
- Fair comparison between RF and baselines on identical balanced data

update-6 - 28072025
- Auto feature engineering
- Complete feature alignment
- Enhanced error logging
- RF vs Competitor advantage analysis
...
#!/usr/bin/env python3
"""
FINAL FIX: Model Comparison - Uses EXACT same data as training
==============================================================
Loads data exactly as used in training, no synthetic generation
"""
...
class FixedModelComparison:
    ...
    def create_engineered_features(self, df):
        """
        CRITICAL FIX: Create ALL engineered features exactly as used in training
        """
        ...
        logger.info(f"Engineered features created. Total columns: {len(df_eng.columns)}")
    ...
    def apply_balanced_preprocessing(self, X, y, required_features, logger):
        ...
        if missing_features:
            ...
            # Create dummy features with zeros (safer than dropping)
            for feature in missing_features:
                X[feature] = 0.0
                logger.info(f"Created dummy feature: {feature}")
    ...
    def evaluate_algorithms_on_period(self, period_id: str, rf_model_package: dict):
        ...
        try:
            ...
            logger.warning(" SMOTE failed, using original training data")
